...
; Creates the line of ink dots between two points. Used by move-pen-to when the pen is down.
(defun draw-line (xpos1 ypos1 xpos2 ypos2 &optional (delta 1))
  (multiple-value-bind (roundedxpos xremainder) (round xpos1)
    ...
    (incf ink-num*)
  (cond ((and (equal xpos1 xpos2) (< ypos1 ypos2))
	 (draw-line xpos1 (+ ypos1 1) xpos2 ypos2))
	((and (equal xpos1 ypos2) (> ypos1 ypos2))
	 (draw-line xpos1 (- ypos1 1) xpos2 ypos2))
	((< xpos1 xpos2)
...
(defun *move-pen-to (xpos2 ypos2)
  (let* ((object (assoc 'pen state*))
	 ...
	 (status (member 'status object)))
    (if (and (equal (cadr status) 'down)
	     (or (not (equal (cadr xpos1) xpos2))
		 (not (equal (cadr ypos1) ypos2))))
